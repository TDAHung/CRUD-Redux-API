...
const samplePageSlice = createSlice({
    ...
    reducers:{
        ...
        editData:(state: IinitialState, action: {type:string, payload: {dataSubmit: IValueSample, isEdit: number}})=>{
            if(action.payload.isEdit > -1){
                state.data[action.payload.isEdit] = action.payload.dataSubmit;
            }
        },
        // some actions of the redux
    }
...
export const middleware = () : ThunkAction<void, RootState, unknown, AnyAction> =>{
    return async (dispatch, getState) => {
        ...
        dispatch(loadInitialData((await (await methodAPI.getAll()).data))); // load the data from the API first
